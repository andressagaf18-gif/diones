...
function extrairOutputText(data) {
  if (
    ...
    return data.output_text.trim();// api/diagnostico.js
// Finder — diagnóstico v3 otimizado.
...
function lerCache(chave) {
  const item =
    cacheDiagnostico.get(chave);

  if (!item) return null;

  if (Date.now() - item.ts > CACHE_TTL_MS) {
    cacheDiagnostico.delete(chave);
    return null;
  }
...
function gravarCache(chave, data) {
  cacheDiagnostico.set(chave, {
    ts: Date.now(),
    data,
  });

  if (cacheDiagnostico.size > 150) {
    const primeiro =
      cacheDiagnostico.keys().next().value;

    cacheDiagnostico.delete(primeiro);
  }
}
